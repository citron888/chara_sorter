...
過去に入力したキャラ名の順番を記録し、AI認識結果の候補をその順に並べ替えて表示します。  
AIモデル自体の再学習は行いません（設定でOFF可能）。
...
### 重複ファイルの自動削除
移動先フォルダに **MD5ハッシュが完全一致するファイルが存在する場合**、元ファイルは自動的に**ゴミ箱**へ移動されます。  
同じ画像を誤って2回移動しようとした場合などに発生します。ゴミ箱から復元可能です。  
この動作は設定の **重複チェック** をOFFにすることで無効化できます。
